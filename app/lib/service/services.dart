...
Future<R> withFakeServices<R>({
  ...
  return await fork(() async {
        ...
        if (configuration == null) {
          ...
          final frontendServerUri = Uri.parse(
            'http://localhost:${frontendServer.server.port}',
          );
        ...
        return await _withPubServices(() async {
          ...
          if (frontendServer != null) {
            final handler = wrapHandler(
              _logger,
              _fakeClockWrapper(createAppHandler()),
              sanitize: true,
            );
  ...
  return fork(() async {
        ...
        registerSearchBackend(
          SearchBackend(
            ...
            storageService.bucket(
              activeConfiguration.searchSnapshotBucketName!,
            ),
          ),
        );
        ...
        registerPackageBackend(
          PackageBackend(
            ...
            storageService,
            storageService.bucket(
              activeConfiguration.incomingPackagesBucketName!,
            ),
            storageService.bucket(
        ...
        return await fork(
          () async => Zone.current
              .fork(zoneValues: {_pubDevServicesInitializedKey: true})
              .run(() async {
                ...
                try {
                  ...
                  _logger.severe(
                    'Uncaught exception inside services scope.',
                    e,
                    st,
                  );
                  rethrow;
